...
class Deal extends AppModel {
	...
	public function checkEmptyEmail($data) {
		$data['recipient_email'] = trim($data['recipient_email']);
	...
	public function addNewDeal($dealInfo) {
		// Get MasterDoc files_id for this deal
		$files_id = $this->MasterDoc->find('first', array( 'conditions' => array('MasterDoc.id' => $dealInfo['Deal']['master_docs_id']),
			'fields' => array('files_id')
		));
		...
		$redlineData[$redline->name] = array(
						...
						'files_id' => $files_id['MasterDoc']['files_id']
			);
        ...
        /**
	 * @name 		getEmailDoc 
	 * @description		getting deals file from s3         			
	 * @parm	        $sessDealData
	 * @return	        boolean	 
	 */
        public function getEmailDoc($sessDealData){
                //Start Code to save file from S3 server to local Server
                $this->AmazoneFileSystem = ClassRegistry::init('AmazoneFileSystem');                          
                $this->AmazoneFileSystem->getObject($this->AmazoneFileSystem->bucket, Configure::read('AWS_DOC_FOLDER') . $sessDealData['Deal']['master_docs_file_path']. $sessDealData['Deal']['master_docs_file'], Configure::read('EMAIL_DOC_PATH').$sessDealData['Deal']['master_docs_file']);
                if($this->AmazoneFileSystem->getObject($this->AmazoneFileSystem->bucket, Configure::read('AWS_DOC_FOLDER') . $sessDealData['Deal']['master_docs_file_path']. $sessDealData['Deal']['master_docs_file'], Configure::read('EMAIL_DOC_PATH').$sessDealData['Deal']['master_docs_file'])) {
                return true;
            } else {
                return false;
            }
