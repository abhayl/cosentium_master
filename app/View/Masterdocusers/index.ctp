...
<div class="heading"><?php echo __('Master Documents');?></div>
	<div class="userlistcontainer">    
          ...
          <tr>
            ...
            <?php echo $this->Form->create('MasterdocUser', array('controller' => 'masterdocusers','action' => 'index'));?>
            <td width="39%">
          ...
          <tbody>
              ...
              foreach ($masterDoc as $masterDoc):
                  ...
                  <?php }else{ ?>
                      <tr>
                  <?php }
                  $masterDocName = explode('.docx', $masterDoc['MasterdocUser']['name']);        
                if($masterDoc['MasterdocUser']['owner'] == CakeSession::read('user.User.id')){ ?>
                          <td align="left" class="roles_permission_tblcol1"><a href="#" onClick="javascript:submitForm(<?php echo $masterDoc['MasterdocUser']['files_id'] ?>);"><?php echo __($masterDocName[0]);?></a><?php //echo $this->Html->link(__($masterDocName[0]), array('action' => '#', 'onclick' => 'javascript:submitForm();'));?></td>
                <?php }else{ ?>
                ...
                <td align="left" class="roles_permission_tblcol1"><?php echo $this->Html->image($amazonS3ServerPath.$profileImage, array('class' => "usrthumbnail_right_margin"));?><?php echo $masterDoc['User']['last_name'].', '.$masterDoc['User']['first_name'];?></td>
                <td align="center">
                    <?php if($masterDoc['MasterdocUser']['available_after_dt'] != NULL){
                        echo date(Configure::read('DATE_FORMAT'), strtotime($masterDoc['MasterdocUser']['available_after_dt']));
                    }else{
                        echo $masterDoc['MasterdocUser']['available_after_dt'];
                    }?>
                </td>
                <td align="center">
                    <?php if($masterDoc['MasterdocUser']['available_after_dt'] != NULL){
                        echo date(Configure::read('DATE_FORMAT'), strtotime($masterDoc['MasterdocUser']['available_until_dt']));
                    }else{
                        echo $masterDoc['MasterdocUser']['available_until_dt'];
                    }?>
                </td>
              </tr>
              <?php $i++; endforeach; ?>
